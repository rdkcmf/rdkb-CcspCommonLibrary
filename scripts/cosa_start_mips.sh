...
NOTIFYPRESENT=`cat /tmp/bbhm_cur_cfg.xml | grep NotifyWiFiChanges`
REDIRCTEXISTS=""
...
then
	...
	then
		...
		sed '10 a \ \ \ <Record name=\"eRT.com.cisco.spvtg.ccsp.Device.WiFi.NotifyWiFiChanges\" type=\"astr\">false</Record>' /tmp/bbhm_cur_cfg.xml > /var/tmp/bbhm_cur_cfg.xml
		...
		cp /var/tmp/bbhm_cur_cfg.xml /tmp/bbhmm_cur_cfg.xml
	...
	then
		...
		sed '10 a \ \ \ <Record name=\"eRT.com.cisco.spvtg.ccsp.Device.WiFi.NotifyWiFiChanges\" type=\"astr\">true</Record>' /tmp/bhm_cur_cfg.xml > /var/tmp/bbhm_cur_cfg.xml
		...
		cp /var/tmp/bbhm_cur_cfg.xml /tmp/bhm_cur_cfg.xml
		cp /var/tmp/bbhm_bak_cfg.xml /nvram/bbhm_bak_cfg.xml
		rm /var/tmp/bbhm_cur_cfg.xml
		rm /var/tmp/bbhm_bak_cfg.xml
	fi
...
if [ "$?" == "1" ];then
	...
	if [ "$IS_BCI" = "yes" ]; then
		...
		grep "<Record name=\"eRT.com.cisco.spvtg.ccsp.tr181pa.Device.WiFi.AccessPoint.9.BssMaxNumSta\" type=\"astr\">5<\/Record>" /tmp/bbhm_cur_cfg.xml
                if [  "$?" == "1" ] ; then
			cp /tmp/bbhm_cur_cfg.xml /tmp/b1
