...
cd /etc
chmod +x xf3_firmwareDwnld.sh
./xf3_firmwareDwnld.sh &
...
cd /usr/ccsp
if [ -e /nvram/disableCcspEPONAgentSsp ]; then
   echo "****DISABLE CcspEPONAgentSsp*****"
elif [ -e ./epon ]; then
        cd epon
        if [ "x"$Subsys = "x" ];then
        $BINPATH/CcspEPONAgentSsp
        else
        echo "$BINPATH/CcspEPONAgentSsp -subsys $Subsys"
                $BINPATH/CcspEPONAgentSsp -subsys $Subsys
        fi
        cd ..
fi
