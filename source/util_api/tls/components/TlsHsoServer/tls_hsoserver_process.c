...
/**********************************************************************
    ...
    module:	tls_hsoserver_process.c
    ...
    )
{
    ...
    while ( ulRestSize >= sizeof(TLS_HANDSHAKE_HEADER) )
    {
        ...
        if ( TlsHandshakeGetMsgType(pTlsHandshakeHeader) != TLS_HANDSHAKE_TYPE_finished )
        {
            if ( pSsl2RecordHeader )
            {
                returnStatus =
                    pMyObject->SaveHandshakeMsg
                        (
                            (ANSC_HANDLE)pMyObject,
                            Ssl2RecordGetData  (pSsl2RecordHeader),
                            Ssl2RecordGetLength(pSsl2RecordHeader)
                        );
            }
            else
            {
        ...
        switch ( TlsHandshakeGetMsgType(pTlsHandshakeHeader) )
        {
            ...
            case    TLS_HANDSHAKE_TYPE_clientKeyExchange :
                    ...
                    returnStatus =
                        pMyObject->RecvClientKeyExchange
                            (
                                (ANSC_HANDLE)pMyObject,
                                TlsHandshakeGetBody  (pTlsHandshakeHeader),
                                TlsHandshakeGetLength(pTlsHandshakeHeader)
            ...
            case    TLS_HANDSHAKE_TYPE_certificateVerify :

                    returnStatus =
                        pMyObject->RecvCertVerify
                            (
                                (ANSC_HANDLE)pMyObject,
                                TlsHandshakeGetBody  (pTlsHandshakeHeader),
                                TlsHandshakeGetLength(pTlsHandshakeHeader)
